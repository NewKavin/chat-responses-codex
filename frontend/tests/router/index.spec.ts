...
describe('router', () => {
  ...
  it('removes the troubleshooting center from both shells', () => {
    expect(router.getRoutes().some(route => route.path === '/portal/troubleshooting')).toBe(false)
    expect(router.getRoutes().some(route => route.path === '/admin/troubleshooting')).toBe(false)
    expect(router.getRoutes().some(route => route.name === 'PortalTroubleshooting')).toBe(false)
    expect(router.getRoutes().some(route => route.name === 'AdminTroubleshooting')).toBe(false)
  })
