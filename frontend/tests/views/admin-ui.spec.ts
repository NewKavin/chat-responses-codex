...
describe('admin ui structure', () => {
  ...
  it('supports editable visible column preferences on upstream and downstream lists', () => {
    ...
    expect(upstream).toContain("key: 'key_concurrency', label: '每 Key 最大并发'")
    expect(upstream).toContain("v-if=\"isColumnVisible('key_concurrency')\"")
    expect(upstream).toContain('{{ row.max_concurrency }}')
    ...
    // T13：模型权限列只显示分组（model_allowlist 已停写）
    expect(downstream).toContain('row.model_group_id')
    expect(downstream).not.toContain('formatModelList(row.model_allowlist)')
