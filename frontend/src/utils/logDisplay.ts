...
export const errorCategoryGroups: ErrorCategoryGroup[] = [
  ...
  },
  {
    ...
    options: [
      ...
      { value: 'stream_upstream_body_decode_error', label: '上游响应解码失败（旧码）' },
      { value: 'stream_upstream_transport_decode_error', label: '上游响应解码失败（传输层）' },
      { value: 'stream_upstream_sse_parse_error', label: '上游 SSE 解析失败' },
