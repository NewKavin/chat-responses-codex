...
#[tokio::test]
async fn v1_models_endpoint_omits_xhigh_for_deepseek_v4_pro() {
    let dir = tempdir().unwrap();
    let state_path = dir.path().join("state.json");
    let downstream_key = generate_downstream_key("gw");
    let model_slug = "deepseek-ai/deepseek-v4-pro";
    let state = AppState::new(
        PersistedState {
            upstreams: vec![UpstreamConfig {
                id: "up-1".into(),
...
#[tokio::test]
async fn v1_models_endpoint_omits_xhigh_for_deepseek_v4_flash() {
    let dir = tempdir().unwrap();
    let state_path = dir.path().join("state.json");
    let downstream_key = generate_downstream_key("gw");
    let model_slug = "deepseek-v4-flash";
    let state = AppState::new(
        PersistedState {
            upstreams: vec![UpstreamConfig {
                id: "up-1".into(),
