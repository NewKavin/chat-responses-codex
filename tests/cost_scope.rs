//! 费用归属解析（cost scope）集成测试：通过 crate 公共 API 验证。
//! （state 模块的内部单元测试在本仓库无法被 cargo 编译运行，见
//!  tests/unit/state/cost_scope.rs 的说明；这里用公共 API 覆盖同一语义。）

use chat_responses_codex::state::{resolve_cost_scope, CostScope};
...
fn scope_without_a_configured_limit_is_unlimited() {
    let scope = resolve_cost_scope("key-a", &owners(&[("key-a", "user-1")]), &HashMap::new());
    assert_eq!(scope.scope_id, "user-1");
...
#[test]
fn cost_scope_is_limited_reflects_configured_limit() {
    let limited = CostScope {
        scope_id: "user-1".into(),
        daily_limit_cents: Some(100),
    };
    assert!(limited.is_limited());
    let unlimited = CostScope {
        scope_id: "user-1".into(),
