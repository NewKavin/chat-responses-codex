...
fn unique_state_path() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    PathBuf::from(format!("/tmp/test_state_admin_downstreams_{nanos}.json"))
}
...
async fn test_downstreams_update_modifies_existing_downstream() {
    ...
    let response = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header(header::AUTHORIZATION, format!("Bearer {}", token))
                .header(header::CONTENT_TYPE, "application/json")
                .body(Body::from(serde_json::to_string(&updated_downstream).unwrap()))
                .unwrap(),
        )
        .await
        .unwrap();
    ...
    let snapshot = state.snapshot().await;
    let downstream = snapshot.downstreams.iter().find(|d| d.id == "downstream-1").unwrap();
...
async fn test_downstreams_update_preserves_key_hash() {
    ...
    let original_hash = {
        let snapshot = state.snapshot().await;
        snapshot.downstreams.iter().find(|d| d.id == "downstream-1").unwrap().hash.clone()
    };
...
async fn test_downstreams_toggle_changes_active_status() {
    ...
    let snapshot = state.snapshot().await;
    let downstream = snapshot.downstreams.iter().find(|d| d.id == "downstream-1").unwrap();
    assert_eq!(downstream.active, false);
}
