...
async fn test_list_user_accessible_groups_includes_binding_groups() {
    ...
    client
        .execute(
            "INSERT INTO portal_user_downstreams (user_id, downstream_id, label, model_group_id, is_default, created_at) \
             VALUES ($1, $2, 'Bound Key', 'premium', FALSE, NOW())",
            &[&user_id, &"sk-test-bound"],
        )
        .await
    ...
    let groups = store
        .list_user_accessible_model_groups(user_id)
        .await
        .expect("list groups after binding");
