        ...
        post({
            ...
            move |request: Request<Body>| {
                ...
                async move {
                    ...
                    (
                        [(header::CONTENT_TYPE, "text/event-stream")],
                        "data: {\"id\":\"chatcmpl-test\",\"object\":\"chat.completion.chunk\",\"created\":1,\"model\":\"GLM-5.1\",\"choices\":[{\"index\":0,\"delta\":{\"content\":\"OK\"},\"finish_reason\":null}]}\n\ndata: {\"id\":\"chatcmpl-test\",\"object\":\"chat.completion.chunk\",\"created\":1,\"model\":\"GLM-5.1\",\"choices\":[{\"index\":0,\"delta\":{},\"finish_reason\":\"stop\"}]}\n\ndata: [DONE]\n\n",
                    )
                        .into_response()
                }
            }
...
#[tokio::test]
async fn portal_troubleshooting_preserves_http_200_structured_stream_error_category() {
    let upstream = spawn_semantically_invalid_diagnostic_upstream().await;
    let (app, portal_key, _) = app_with_custom_upstream(upstream);
    let token = login_portal(app.clone(), &portal_key).await;
    ...
    assert_eq!(result["http_status"], StatusCode::OK.as_u16());
    assert_eq!(result["error_category"], "upstream_stream_error_event");
}
...
async fn portal_troubleshooting_runs_chat_stream_and_tools_checks() {
    ...
    assert_eq!(
        tool_request.body["tool_choice"]["function"]["name"],
        "diagnostic_echo"
    );
    assert_eq!(
...
async fn portal_troubleshooting_stream_check_has_diagnostic_timeout() {
    ...
    let (app, portal_key, _) =
        app_with_custom_upstream_and_ip_allowlist_and_config(upstream_base_url, vec![], config);
    let token = login_portal(app.clone(), &portal_key).await;
