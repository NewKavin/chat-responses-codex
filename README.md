...
当前示例按 Codex CLI `0.146.0` 验证，并默认启用 `multi_agent`。调整 `[agents]` 下的 `max_threads` 可以增加并发代理线程数，`max_depth` 用于限制嵌套委派深度；这些客户端设置不会覆盖网关配额。`~/.codex/agents/default.toml` 不要写 `model` 和 `model_reasoning_effort`：子代理角色留空这两项即继承主配置，切换模型后不需要替换它，然后运行 `codex login --with-api-key` 写入下游 key，并用 `codex --strict-config doctor --summary` 检查配置是否实际生效。
...
The sample is validated against Codex CLI `0.146.0`. Leave `model` and `model_reasoning_effort` out of `~/.codex/agents/default.toml`: the subagent role inherits the main config, so switching models needs no edit in that file. Run `codex login --with-api-key`, tune `[agents].max_threads` for concurrent agent threads and `[agents].max_depth` for nested delegation, then run `codex --strict-config doctor --summary` to validate the loaded configuration.
