...
use chat_responses_codex::state::{
    ...
    DEFAULT_PORTAL_SESSION_TTL_SECONDS, DEFAULT_ROUTE_HEALTH_HALF_OPEN_EXCLUSIVE_WINDOW_MS,
    DEFAULT_ROUTE_HEALTH_HALF_OPEN_TTL_SECONDS, DEFAULT_STREAM_DECODE_ERROR_CODE_SPLIT_ENABLED,
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let portal_oidc_userinfo_method = env_or(
        "PORTAL_OIDC_USERINFO_METHOD",
        DEFAULT_PORTAL_OIDC_USERINFO_METHOD,
    );
    let portal_oidc_token_path = env_or("PORTAL_OIDC_TOKEN_PATH", DEFAULT_PORTAL_OIDC_TOKEN_PATH);
    let portal_oidc_uuid_field = env_or("PORTAL_OIDC_UUID_FIELD", DEFAULT_PORTAL_OIDC_UUID_FIELD);
    ...
    let stream_max_skipped_bad_frames = env_u64(
    ...
    let mut config = AppConfig {
        ...
        upstream_rate_limit_internal_retry_enabled: env_bool(
