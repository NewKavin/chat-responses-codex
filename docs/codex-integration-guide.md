...
若所有候选都因上游限流、并发饱和或 Key 配额耗尽，网关返回 `429 upstream_routes_exhausted`（`rate_limit_error`）；如果临时耗尽中混有 5xx、网络错误或普通容量不足，则仍返回 503。两种响应的 `Retry-After` 都取健康注册表中的实时最早恢复时间，安全错误文案还会给出原因、路由数量和网关已经重试的时间。此时应检查该模型是否配置了足够的独立 route，以及各 route 的 Key 模型映射和实时容量。
...
- 如果上游因为并发满返回 429，网关会立即冷却精确 route 并切换候选；请求内恢复预算允许时会按短探测序列重试，预算耗尽后 Codex 按终态 429 中的 `Retry-After` 自动等待。
