...
| F3.1 | 重试放大按类别记录 + API 返回 `category` | `ac51be4`, `fe00a29d`, `24689a9` | ✅ 按 `(downstream_id, model, category)` 独立计数；API `points` 返回稳定的 `gateway_gate` / `routes_exhausted` / `upstream_429` 分类，并显式返回 `category_totals` 分类小计；单测验证三类互不串台及小计汇总 |
